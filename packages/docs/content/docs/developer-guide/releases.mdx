...
description: How Getcito handles versioning, changesets, and the release process.
---

## Changesets

Getcito uses [Changesets](https://github.com/changesets/changesets) for version management. All packages in the monorepo use fixed versioning — they stay on the same version number.
...
|----------|----------|-------------|
| `getcito-cli` | npm | The Getcito CLI package |
| `ai-search-guru/getcito-worlds-first-open-source-aio-aeo-or-geo-tool-web` | Docker Hub | Web app Docker image |
| `ai-search-guru/getcito-worlds-first-open-source-aio-aeo-or-geo-tool-worker` | Docker Hub | Worker Docker image |
| `ai-search-guru/getcito-worlds-first-open-source-aio-aeo-or-geo-tool-db-migrate` | Docker Hub | One-shot drizzle migrations image |
